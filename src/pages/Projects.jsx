...
import {
    ...
    Loader2, Clock, Paperclip, FileText,
...
export default function Projects() {
    ...
    if (loading && view === "list" && projects.length === 0) {
        return <div className="p-20 text-center text-muted">Loading projects...</div>;
    }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-6">
                <div>
                    {view === "detail" ? (
                        ...
                        <p className="text-muted text-sm uppercase tracking-wide font-semibold">Research / Projects</p>
                    )}
                ...
                {view === "list" && (
                    <button
                        ...
                        className="flex items-center gap-2 bg-accent text-white px-4 py-2 rounded-xl hover:opacity-95 transition shadow-lg shadow-blue-500/20 font-medium"
                    >
                ...
                <>
                    {projects.length === 0 ? (
                        <div className="bg-surface border-2 border-dashed border-border rounded-2xl p-12 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-fg">No projects yet</h3>
                            <p className="text-muted mb-6">Create a project to track your research milestones.</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {projects.map((project) => (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-muted mt-3 line-clamp-2 h-10">
                                        {project.description}
                                    </p>
...
function ProjectDetailView({ project, setProject }) {
    ...
    return (
        <div className="bg-surface rounded-2xl border border-border shadow-sm overflow-hidden min-h-[600px] flex flex-col md:flex-row">

            <div className="w-full md:w-64 bg-app border-r border-border p-4 flex flex-col gap-2">
                ...
                {activeTab === 'overview' && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="flex justify-between items-start">
                            <div>
                                <h2 className="text-2xl font-bold text-fg">{project.name}</h2>
                                <p className="text-muted mt-1">Project ID: {project._id}</p>
                            </div>
                            <StatusBadge status={project.status} />
                        </div>

                        <div className="p-4 bg-accent-soft rounded-xl border border-border text-fg">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-app rounded-xl border border-border">
                                <span className="text-xs font-bold text-muted uppercase">Start Date</span>
                                <p className="text-lg font-medium text-fg">{new Date(project.startDate).toLocaleDateString()}</p>
                            </div>
                            <div className="p-4 bg-app rounded-xl border border-border">
                                <span className="text-xs font-bold text-muted uppercase">End Date</span>
...
function TeamSection({ project, onUpdate }) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <h3 className="text-xl font-bold text-fg">Team Members</h3>

            <form onSubmit={handleAddMember} className="flex gap-2">
                <input
                    type="email"
                    placeholder="Enter colleague's email"
                    className="flex-1 px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg placeholder:text-muted"
                    value={email}
                    onChange={e => setEmail(e.target.value)}
                />
                <button disabled={loading} className="bg-accent text-white px-4 py-2 rounded-lg hover:opacity-95 transition disabled:opacity-50">
            ...
            <div className="space-y-2">
                {project.teamMembers.length === 0 ? (
                    <p className="text-muted text-center py-4">No team members added yet.</p>
                ) : (
                    project.teamMembers.map((member, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 bg-app rounded-lg border border-border">
                            <div className="bg-blue-100 text-blue-600 p-2 rounded-full"><Users size={16}/></div>
                            <span className="text-fg font-medium">{member}</span>
                        </div>
                    ))
                )}
            </div>
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold text-fg">Attachments</h3>
            </div>

            <form onSubmit={handleUpload} className="p-4 bg-app rounded-xl border border-border space-y-3">
                <input
                    type="text"
                    placeholder="File Display Name"
                    className="w-full px-3 py-2 rounded-lg border border-border bg-surface text-fg placeholder:text-muted focus:outline-none focus:border-accent"
                    value={fileName}
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="bg-accent text-white px-4 py-2 rounded-lg hover:opacity-95 disabled:opacity-50 flex items-center gap-2"
                    >
            ...
            {loading ? (
                <p className="text-center text-muted py-4">Loading files...</p>
            ) : (
                <div className="grid gap-3">
                    {files.length === 0 ? (
                        <p className="text-muted text-center py-8">No files attached.</p>
                    ) : (
                        files.map((file) => (
                            <div key={file._id} className="flex items-center justify-between p-3 bg-surface border border-border rounded-xl hover:shadow-sm transition">
                       ...
                       Uploaded: {new Date(file.createdAt).toLocaleDateString()}
                                ...
                                <a href={getSecureUrl(file.url)} target="_blank" rel="noreferrer" className="text-accent hover:opacity-95 p-2">
                                    <Download size={18} />
                                </a>
                            </div>
                        ))
...
function NotesSection({ projectId }) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <h3 className="text-xl font-bold text-fg">Project Notes</h3>

            <form onSubmit={handleAddNote} className="relative">
        <textarea
            rows="3"
            placeholder="Type a note..."
            className="w-full p-4 rounded-xl border border-border bg-app focus:ring-2 focus:ring-ring outline-none pr-12 text-fg placeholder:text-muted"
            value={noteText}
            onChange={e => setNoteText(e.target.value)}
        />
                <button
                    disabled={submitting || !noteText.trim()}
                    className="absolute bottom-3 right-3 p-2 bg-accent text-white rounded-lg hover:opacity-95 disabled:opacity-50 transition"
                >
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                {loading ? <p className="text-center text-muted">Loading notes...</p> :
                    notes.length === 0 ? (
                        <p className="text-muted text-center py-4">No notes yet.</p>
                    ) : (
                        notes.map((n) => (
                            <div key={n._id} className="bg-surface p-4 rounded-xl border border-border text-fg text-sm">
                                <p className="leading-relaxed text-muted">{n.content}</p>
                                <div className="mt-2 pt-2 border-t border-border flex flex-col gap-0.5 text-xs text-muted">
    ...
    return (
        <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all text-left ${
                active ? "bg-surface text-accent shadow-sm" : "text-muted hover:bg-surface2 hover:text-fg"
            }`}
        >
            {icon} {label}
        </button>
...
function ProjectFormModal({ projectToEdit, onClose, onSuccess }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-surface rounded-2xl w-full max-w-lg p-6 relative shadow-2xl border border-border">
                <button onClick={onClose} className="absolute top-4 right-4 text-muted hover:text-fg"><X size={24} /></button>
                <h2 className="text-xl font-bold text-fg mb-6">{isEdit ? "Edit Project" : "New Project"}</h2>

                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-muted mb-1">Name</label>
                        <input required name="name" type="text" className="w-full px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg placeholder:text-muted"
                               value={formData.name} onChange={handleChange} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-muted mb-1">Start Date</label>
                            <input required name="startDate" type="date" className="w-full px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg"
                                   value={formData.startDate} onChange={handleChange} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-muted mb-1">End Date</label>
                            <input name="endDate" type="date" className="w-full px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg"
                    ...
                    <div>
                        ...
                        <textarea required name="description" rows="3" className="w-full px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg placeholder:text-muted"
                                  value={formData.description} onChange={handleChange} />
                    </div>
                    {!isEdit && (
                        <div>
                            <label className="block text-sm font-medium text-muted mb-1">Team (Comma separated emails)</label>
                            <input name="teamMembers" type="text" className="w-full px-4 py-2 rounded-lg border border-border bg-app focus:ring-2 focus:ring-ring outline-none text-fg placeholder:text-muted"
                                   value={formData.teamMembers} onChange={handleChange} placeholder="email1, email2..." />
                        </div>
                    )}
                    <button type="submit" disabled={loading} className="w-full bg-accent text-white py-3 rounded-xl font-medium hover:opacity-95 transition flex justify-center items-center gap-2 disabled:opacity-70">
